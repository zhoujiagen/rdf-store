...
import com.spike.giantdataanalysis.rdfstore.commons.lang.MoreBytes.LexicographicalComparerHolder.UnsafeComparer;

import sun.misc.Unsafe;

/**
 * byte utility class copied from HBase.
 */
...
public class MoreBytes {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(MoreBytes.class);
  ...
  public static int getInt(byte[] b, int offset) {
    byte[] value = copy(b, offset, Integer.SIZE / Byte.SIZE);
    return toInt(value);
  }

  public static long getLong(byte[] b, int offset) {
    byte[] value = copy(b, offset, Long.SIZE / Byte.SIZE);
    return toLong(value);
  }
  ...
  public static int putLongUnsafe(byte[] bytes, int offset, long val) {
    ...
    UnsafeComparer.theUnsafe.putLong(bytes, (long) offset + UnsafeComparer.BYTE_ARRAY_BASE_OFFSET,
      val);
    return offset + SIZEOF_LONG;
  }
  ...
  public static byte[] toBytes(final float f) {
    // Encode it as int
    return MoreBytes.toBytes(Float.floatToRawIntBits(f));
  }
  ...
  public static byte[] toBytes(final double d) {
    // Encode it as a long
    return MoreBytes.toBytes(Double.doubleToRawLongBits(d));
  }
  ...
  public static int putIntUnsafe(byte[] bytes, int offset, int val) {
    ...
    UnsafeComparer.theUnsafe.putInt(bytes, (long) offset + UnsafeComparer.BYTE_ARRAY_BASE_OFFSET,
      val);
    return offset + SIZEOF_INT;
  }
  ...
  public static int putShortUnsafe(byte[] bytes, int offset, short val) {
    ...
    UnsafeComparer.theUnsafe.putShort(bytes, (long) offset + UnsafeComparer.BYTE_ARRAY_BASE_OFFSET,
      val);
    return offset + SIZEOF_SHORT;
  }
   ...
   *         method is what {@link org.apache.hadoop.io.Text} and
  ...
  public static byte[][] toByteArrays(final String[] t) {
    ...
    for (int i = 0; i < t.length; i++) {
      result[i] = MoreBytes.toBytes(t[i]);
    }
    return result;
  }
  ...
  public static byte[][] toBinaryByteArrays(final String[] t) {
    byte[][] result = new byte[t.length][];
    for (int i = 0; i < t.length; i++) {
      result[i] = MoreBytes.toBytesBinary(t[i]);
    }
    return result;
  }
  ...
  public static boolean equals(List<byte[]> a, List<byte[]> b) {
    ...
    for (int i = 0; i < a.size(); ++i) {
      if (!MoreBytes.equals(a.get(i), b.get(i))) {
        return false;
      }
    }
    return true;
  ...
  // Lists.newArrayListWithCapacity(CollectionUtils.nullSafeSize(strings));
   ...
   * {@code java.util.Arrays.copyOfRange(array, i, i + target.length)} contains exactly the same
   * elements as {@code target}.
